...
class grid_public_unidade_xml
{
   ...
   //---- 
   function grid_public_unidade_xml()
   {
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo     .= ".xml";
      $this->Tit_doc      = "grid_public_unidade.xml";
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_public_unidade']['xml_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_public_unidade']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_public_unidade']['xml_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_public_unidade']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      while (!$rs->EOF)
      {
         $this->xml_registro = "<grid_public_unidade";
   ...
   function NM_export_id_unidade()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->id_unidade))
         {
             $this->id_unidade = sc_convert_encoding($this->id_unidade, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " id_unidade =\"" . $this->trata_dados($this->id_unidade) . "\"";
   }
   //----- ds_unidade
   function NM_export_ds_unidade()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->ds_unidade))
         {
             $this->ds_unidade = sc_convert_encoding($this->ds_unidade, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " ds_unidade =\"" . $this->trata_dados($this->ds_unidade) . "\"";
   }
   //----- ds_unidade_antiga
   function NM_export_ds_unidade_antiga()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->ds_unidade_antiga))
         {
             $this->ds_unidade_antiga = sc_convert_encoding($this->ds_unidade_antiga, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " ds_unidade_antiga =\"" . $this->trata_dados($this->ds_unidade_antiga) . "\"";
   }
   //----- cd_unidade
   function NM_export_cd_unidade()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->cd_unidade))
         {
             $this->cd_unidade = sc_convert_encoding($this->cd_unidade, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " cd_unidade =\"" . $this->trata_dados($this->cd_unidade) . "\"";
   }
   //----- ds_unidade_compl
   function NM_export_ds_unidade_compl()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->ds_unidade_compl))
         {
             $this->ds_unidade_compl = sc_convert_encoding($this->ds_unidade_compl, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " ds_unidade_compl =\"" . $this->trata_dados($this->ds_unidade_compl) . "\"";
   }
   //----- id_bloco
   ...
   function NM_export_andar()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->andar))
         {
             $this->andar = sc_convert_encoding($this->andar, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " andar =\"" . $this->trata_dados($this->andar) . "\"";
   }
   //----- id_pessoa_fisica
   function NM_export_id_pessoa_fisica()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->id_pessoa_fisica))
         {
             $this->id_pessoa_fisica = sc_convert_encoding($this->id_pessoa_fisica, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " id_pessoa_fisica =\"" . $this->trata_dados($this->id_pessoa_fisica) . "\"";
   }
   //----- id_pessoa_juridica
   function NM_export_id_pessoa_juridica()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->id_pessoa_juridica))
         {
             $this->id_pessoa_juridica = sc_convert_encoding($this->id_pessoa_juridica, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " id_pessoa_juridica =\"" . $this->trata_dados($this->id_pessoa_juridica) . "\"";
   }
   //----- id_tipo_vinculo
   function NM_export_id_tipo_vinculo()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->id_tipo_vinculo))
         {
             $this->id_tipo_vinculo = sc_convert_encoding($this->id_tipo_vinculo, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " id_tipo_vinculo =\"" . $this->trata_dados($this->id_tipo_vinculo) . "\"";
   }
   //----- fracao
   function NM_export_fracao()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->fracao))
         {
             $this->fracao = sc_convert_encoding($this->fracao, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " fracao =\"" . $this->trata_dados($this->fracao) . "\"";
   }
   //----- obs
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_public_unidade']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_public_unidade']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - Unidade :: XML</TITLE>
